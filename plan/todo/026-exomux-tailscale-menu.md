...
- Still tracked below (unchanged): per-machine actions submenu (system monitor, ping, copy, details), remote
  tmux/exomux session listing with attach/focus-if-open, OSC 7 cwd targeting for scp, capability/provider
  manifest integration, fuzzy filter.

This roadmap adds first-class Tailscale awareness to the Exomux terminal-multiplexer showcase: a `[ Tailnet ]`
...
4. Under each machine, an actions submenu (launch shell, system monitor, …) and a sessions submenu listing
   tmux/exomux sessions on that machine; activating a session opens it in a new terminal or focuses the window
   if it is already open.

## Grounding in the current code

- Menu bar: Exomux renders a row-0 menu bar with `[ New ] [ Sessions ] [ Theme ] [ Help ]`
  (`examples/showcases/exomux/app.ts:122-125`, `app.ts:1561-1572`) driven by `ExomuxMenuId`
...
- Session spawning: the wire protocol already supports arbitrary `command`, `args`, `cwd`, `env`, and `title` in
  `ExomuxSpawnRequest` (`examples/showcases/exomux/protocol.ts:59-68`), so SSH shells, remote monitors, and
...
- Capability/provider seam: `ExomuxClientProvider` (`controller.ts:899`) and the shared showcase provider
  contract (`examples/showcases/shared/provider.ts`) define the degraded/unavailable reporting model.
- Paste plumbing: Exomux already has modal-safe paste handling and the canonical input envelope carries paste
...
- Activating a session row spawns `ssh -t <host> tmux attach -t <name>` (or `tmux new -A -s` per D5) — or the
  exomux attach equivalent — in a new floating window. If a window for that machine+session is already open,
  it is focused instead of duplicated (`Shift-Enter` forces a second attach).
- Machines that are offline, or where the probe finds no tmux/exomux, render an empty-state row ("no sessions")
...
- When a local filesystem path is pasted (or OS-level dropped, which terminals deliver as a paste) onto a focused
  Exomux terminal window whose session was spawned as a Tailnet SSH shell, Exomux intercepts it **before**
...
- Rationale for client-side: the Exomux host is loopback-only (`host.ts:979-981`), so client and daemon are the
...
- `TailnetDevice` type in `model.ts`: id (stable node key hash), shortName, dnsName, ipv4, ipv6, os, online,
  lastSeen, self, tags, relayed. `TailnetRemoteSession` type: machineId, kind (`tmux | exomux`), name, windows,
...
- `ExomuxClientProvider` gains optional capability `network.tailscale` reporting `available` (CLI present,
...
| TSM-015 | Docs + showcase roadmap sync (this file, 025 ledger, api-reference if any public surface moves) | `deno task exomux:check` and repo gates pass; 025 references the TSM block | Gap |
...
- TSM-031 Browser-host panel once Exomux gains a browser host (real drag-and-drop file transfer becomes possible
  there).
...
- **Theme fidelity:** all panel colors resolve through the active Exomux theme (13 themes) — no hardcoded RGB;
...
- **SSH prerequisites are the user's:** if `tailscale ssh` is unsupported and plain `ssh` fails (no key/agent),
  the terminal window shows ssh's own error — Exomux does not manage credentials; the Details modal shows which
...
- **Testing-first:** a `TailnetFixtureSource` (mirroring Inkstone's `fixture_provider.ts`) provides deterministic
  ...
  powering unit, pilot, and snapshot tests without a real tailnet. `deno task exomux:test` grows
  `exomux_tailnet.test.ts`.
...
(`deno task exomux:check`, `exomux:test`, full `deno test`) with fresh ICC evidence and a verified ICC
attempt.
