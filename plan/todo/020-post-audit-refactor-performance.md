...
- Extracted API Workbench HTML/CSS layout demo terminal painting into a focused adapter with direct tests, leaving the
  main workbench renderer to pass retained buffers and theme/write callbacks into the shared layout command pipeline.
...
- Removed single-use API Workbench terminal label/session forwarding wrappers so the adapter calls shared terminal
  status/session helpers directly.
...
- Extracted Three header performance-text projection with direct tests for detailed, compact, and narrow telemetry
  modes; later folded that formatter into `workbench_rows.ts` so header row composition and telemetry text live together
  while preserving the benchmarked `workbench-three-header-telemetry` path.
