...
- [x] Make the terminal workbench and web workbench thin render adapters over the same controller/model.
  ...
  - [x] Extracted shared workbench scrollbar cell projections so terminal and web adapters paint workspace/window
        scrollbars from the same measured glyph coordinates.
  ...
  - [x] Extracted minimized shelf and fullscreen tab source projection into `workbench_shelf.ts`, replacing terminal/
  ...
  - [x] Added shared string-backed frame rectangle filling and migrated the browser API Workbench off its local fill
        loop for themed panel/background rectangles.
...
- [x] Record thresholds in the benchmark catalog and wire the most useful non-flaky cases into health or e2e.
  ...
  - [x] Removed per-selector-match ancestor chain allocation from the CSS cascade by indexing the active node path
        directly during descendant/child selector matching.
...
- [x] Split theme code by responsibility: ANSI/style primitives, component definitions, manifest parsing/validation,
  ...
  - [x] Extracted provider report aggregation behind the public theme facade, injecting coverage, preview, and
        validation collectors so report semantics can be tested without the public facade doing all the work.
