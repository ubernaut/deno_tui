...
| BG-013 | Circuit plausibility: fixpoint reachability grounds every gate to both rails; free-running oscillator (CLK) sources emit square waves and drive nearest gates | done (`circuit_background.ts`, tested) |
...
| BG-017 | Circuit indicator array: eight evenly spaced lamps across the top middle of the board, each fed by its own routed wire on its left pin and preferring a distinct gate, lit from that gate's state on the same synchronous tick; every gate's output drives a gate or a lamp so no node dangles | done (`circuit_background.ts`, tested) |
| BG-016 | Circuit evolution: the board opens as a 3-gate circuit that is already valid (no floating inputs, every cone reaching VCC, GND and CLK) and grows one gate every ~6s, either appended to an existing output or spliced into an existing wire; both moves preserve the invariant, keep the netlist acyclic, and place the gate in its stage band. Destructive periodic re-wiring is gone; a repair pass only runs when the layout despawns or relocates a gate | done (`circuit_background.ts`, tested) |
