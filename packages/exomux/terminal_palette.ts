...
import { decodeTerminalColor } from "@ubernaut/deno-tui/terminal";
...
export function exomuxTerminalForegroundRgb(
  ...
  if (code === undefined) return undefined;
  const color = exomuxTerminalRgb(code, false);
  ...
  if (contrastRatio(preferredText, background) < 4.5) {
    const black: ExomuxRgb = [0, 0, 0];
    const white: ExomuxRgb = [255, 255, 255];
    return contrastRatio(black, background) >= contrastRatio(white, background) ? black : white;
  }
  for (let step = 1; step <= 256; step += 1) {
    const ratio = step / 256;
    const candidate: ExomuxRgb = [
...
function xtermPaletteRgb(index: number): ExomuxRgb {
...
const ANSI_NORMAL: readonly ExomuxRgb[] = Object.freeze([
  [0, 0, 0],
  [205, 49, 49],
  [13, 188, 121],
  [229, 229, 16],
...
const ANSI_BRIGHT: readonly ExomuxRgb[] = Object.freeze([
  [102, 102, 102],
  [241, 76, 76],
  [35, 209, 139],
  [245, 245, 67],
...
function contrastRatio(left: ExomuxRgb, right: ExomuxRgb): number {
...
function relativeLuminance(color: ExomuxRgb): number {
