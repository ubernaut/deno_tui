...
import {
  ...
  type ExomuxInteractiveBackground,
  type ExomuxOverlayBackground,
  type ExomuxOverlayCell,
  mixExomuxRgb,
} from "./background.ts";
 ...
 */
const RAIN_BLUE_TARGET: ExomuxRgb = [80, 140, 220];
...
const WATER_SHALLOW: ExomuxRgb = [96, 170, 235];
const WATER_DEEP: ExomuxRgb = [20, 54, 108];
...
interface RainyPointer extends ExomuxBackgroundPoint {
...
interface MutableOverlayCell {
  column: number;
  row: number;
  readonly cell: { char: string; foreground: ExomuxRgb; bold: boolean };
}

/** Construction options. */
export interface ExomuxRainyWindowsFieldOptions {
 ...
 */
export class ExomuxRainyWindowsField implements ExomuxOverlayBackground, ExomuxInteractiveBackground {
  #randomState: number;
  readonly #density: number;
  #bounds?: Rectangle;
  ...
  readonly #shades: ExomuxRgb[] = [];

  constructor(options: ExomuxRainyWindowsFieldOptions = {}) {
  ...
  rasterizeCells(
    ...
    const rainBright = mixExomuxRgb(mixExomuxRgb(theme.text, RAIN_BLUE_TARGET, 0.45), theme.accent, 0.2);
    const rainBody = mixExomuxRgb(theme.accent, RAIN_BLUE_TARGET, 0.5);
    ...
    for (const drop of this.#drops) {
      ...
      for (let offset = 0; offset <= drop.tail; offset += 1) {
        ...
        if (offset === 0) {
          const foreground = drop.boost > 0 ? mixExomuxRgb(rainBright, theme.text, 0.45 * drop.boost) : rainBright;
        ...
        this.#cells[row]![column] = {
          ...
          foreground: mixExomuxRgb(rainBody, theme.background, fade),
        };
      }
    }
  ...
  rasterizeOverlayCells(
    ...
    for (const splash of this.#splashes) {
      ...
      for (const arc of SPLASH_ARC_TEMPLATES) {
        ...
        this.#emit(col, cellRow, char, mixExomuxRgb(theme.background, rainBright, alpha), alpha > 0.6);
      }
    }
    ...
    if (plug !== undefined) {
      ...
      const rim = mixExomuxRgb(theme.border, theme.background, 0.15);
      const core = this.#drainOpen ? theme.warning : mixExomuxRgb(theme.muted, theme.text, 0.2);
  ...
  #syncShades(theme: ExomuxThemeSpec): void {
    const surface = mixExomuxRgb(WATER_SHALLOW, theme.accent, 0.25);
    const deep = mixExomuxRgb(WATER_DEEP, theme.background, 0.35);
    this.#shades.length = 0;
    for (let index = 0; index < WATER_SHADES; index += 1) {
      this.#shades.push(mixExomuxRgb(surface, deep, index / (WATER_SHADES - 1)));
    }
  }

  #emit(column: number, row: number, char: string, foreground: ExomuxRgb, bold: boolean): void {
