...
export interface ExomuxBiomechFieldOptions {
...
interface BiomechPointer extends ExomuxBackgroundPoint {
 ...
 */
export class ExomuxBiomechField implements ExomuxAnimatedBackground {
  #randomState: number;
  ...
  constructor(options: ExomuxBiomechFieldOptions = {}) {
  ...
  #paintRelief(bounds: Rectangle, theme: ExomuxThemeSpec): void {
    const seconds = this.#timeMs / 1_000;
    for (let y = 0; y < bounds.height; y += 1) {
  ...
  #reliefCell(char: string, level: number, structureMix: number, theme: ExomuxThemeSpec): ExomuxBackgroundCell {
    const structure = mixExomuxRgb(mixExomuxRgb(theme.muted, theme.border, structureMix), theme.surfaceStrong, 0.3);
    let color = mixExomuxRgb(theme.background, structure, 0.15 + 0.75 * level);
    if (level > 0.8) color = mixExomuxRgb(color, theme.accent, (level - 0.8) * 4);
    return level > 0.9 ? { char, foreground: color, bold: true } : { char, foreground: color };
  }

  #paintTubes(bounds: Rectangle, theme: ExomuxThemeSpec): void {
    const seconds = this.#timeMs / 1_000;
    for (const tube of this.#tubes) {
      ...
      for (let y = 0; y < bounds.height; y += 1) {
        ...
        const color = mixExomuxRgb(theme.background, theme.surfaceStrong, 0.3 + 0.5 * level);
  ...
  #paintColumns(bounds: Rectangle, theme: ExomuxThemeSpec): void {
    const seconds = this.#timeMs / 1_000;
    const scroll = Math.floor(seconds * 1.4);
    for (let index = 0; index < this.#anchors.length; index += 1) {
      const anchor = this.#anchors[index]!;
      const structure = mixExomuxRgb(theme.muted, theme.border, anchor.depth);
      for (let y = 0; y < bounds.height; y += 1) {
        ...
        const flank = mixExomuxRgb(theme.background, theme.surfaceStrong, 0.3 + 0.5 * level);
        this.#setCell(bounds, cx - 1, y, { char: "(", foreground: flank });
        this.#setCell(bounds, cx + 1, y, { char: ")", foreground: flank });
        let color = mixExomuxRgb(theme.background, structure, 0.35 + 0.6 * level);
        if (level > 0.78) color = mixExomuxRgb(color, theme.accent, (level - 0.78) * 3);
  ...
  #paintPistons(bounds: Rectangle, theme: ExomuxThemeSpec): void {
    const candidates = this.#anchors.filter((anchor) => anchor.x >= 1 && anchor.x <= bounds.width - 2);
    if (candidates.length === 0) return;
    const housing = mixExomuxRgb(theme.background, theme.border, 0.7);
    const shaft = mixExomuxRgb(theme.background, theme.muted, 0.6);
    for (const piston of this.#pistons) {
      ...
      this.#setCell(bounds, x, headRow, {
        char: "╦",
        foreground: mixExomuxRgb(theme.accent, theme.text, 0.15 + 0.25 * boost),
        bold: true,
      });
    }
  }

  #setCell(bounds: Rectangle, x: number, y: number, cell: ExomuxBackgroundCell): void {
