...
These are real fixed-size terminal captures regenerated with `deno task screenshots`. The set is intentionally limited
to distinct interactive and catalog surfaces; report-style command output stays reproducible through the documented
tasks instead of being checked in as images.
...
layout with wrapped flex rows/columns, a practical CSS Grid subset with `fr` tracks, line spans, and item alignment,
absolute-positioned children, width/height/min/max sizing, padding, margin, borders, overflow, CSS variables,
...
same markup tree through the simple solver, Yoga, or a WorkerPool-backed parse/cascade/layout job. Run
`deno task html-css-workbench` for a colored terminal-cell preview of the shared workbench-style layout fixture. The API
Workbench portfolio also exposes this as a live window: launch `./visualization portfolio` or `deno task api-workbench`,
open `New`, and toggle `Layout: HTML/CSS Layout`.
...
const form = new FormController([
  { name: "route", label: "Route", group: "navigation", initialValue: "overview", validators: [required()] },
]);
...
return disposers, while `setValues()`, `touchAll()`, `setFieldDisabled()`, `setFieldReadOnly()`, `submit()`,
`isDirty()`, `isTouched()`, `isValid()`, and `inspect()` keep multi-field settings panels testable without coupling them
to a concrete widget tree. Fields can carry renderer-neutral `label`, `group`, `disabled`, and `readOnly` metadata;
`inspect()` returns grouped fieldsets, error summaries, disabled/read-only field lists, and submit readiness for custom
forms. Schema adapters can provide field definitions and whole-form validation through `{ fields, validate }`, which
lets apps bridge external validation libraries without tying the form controller to one schema package. `formCommands()`
and `bindFormCommands()` expose submit, validate, reset, touch-all, and optional per-field validate/touch actions
through the shared command registry so form workflows can appear in command palettes, menus, key help, or
plugin-provided surfaces.
...
| `examples/html_css_workbench.ts`        | Colored terminal-cell preview of the HTML/CSS workbench layout |
