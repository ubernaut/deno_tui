...
Deno.test("ThreeAsciiRenderer selects a reduced render profile for block-only terminal frames", () => {
  const renderer = new ThreeAsciiRenderer({
    scene: new Scene(),
    camera: new PerspectiveCamera(),
    columns: 8,
  ...
  const internals = renderer as unknown as {
    resolveRenderProfile(options: { renderAnsi: boolean; renderImage: boolean }): {
      image: boolean;
  ...
  assertEquals(internals.resolveRenderProfile({ renderAnsi: true, renderImage: false }), {
    image: false,
    terminalEdges: false,
...
Deno.test("ThreeAsciiRenderer keeps normals for block terminal frames with depth tinting", () => {
  const renderer = new ThreeAsciiRenderer({
    scene: new Scene(),
    camera: new PerspectiveCamera(),
    columns: 8,
...
Deno.test("ThreeAsciiRenderer keeps full render profile for image and edge glyph frames", () => {
  const renderer = new ThreeAsciiRenderer({
    scene: new Scene(),
    camera: new PerspectiveCamera(),
    columns: 8,
  ...
  assertEquals(internals.resolveRenderProfile({ renderAnsi: false, renderImage: true }), {
    image: true,
    terminalEdges: true,
    terminalDepthColor: true,
  });
