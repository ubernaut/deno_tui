...
import { ExomuxFireField } from "../../examples/showcases/exomux/fire_background.ts";
import { exomuxTheme } from "../../examples/showcases/exomux/model.ts";
...
function grid(field: ExomuxFireField, theme = THEME): string[] {
  return field.rasterizeCells(BOUNDS, theme).map((row) => row.map((cell) => cell?.char ?? " ").join(""));
}

Deno.test("ExomuxFireField: same seed and advance sequence are deterministic", () => {
  const left = new ExomuxFireField({ seed: 21 });
  const right = new ExomuxFireField({ seed: 21 });
  run(left, 200);
  run(right, 200);
  assertEquals(grid(left), grid(right));

  const other = new ExomuxFireField({ seed: 22 });
  run(other, 200);
  assert(grid(other).join("\n") !== grid(left).join("\n"), "different seeds should diverge");
});

Deno.test("ExomuxFireField: matches bounds dimensions and survives resizes", () => {
  const field = new ExomuxFireField({ seed: 3 });
  run(field, 120);
...
Deno.test("ExomuxFireField: burns hottest at the base and cools toward the top", () => {
  const field = new ExomuxFireField({ seed: 9 });
  run(field, 200);
...
Deno.test("ExomuxFireField: flickers between frames rather than sitting still", () => {
  const field = new ExomuxFireField({ seed: 4 });
  run(field, 120);
...
Deno.test("ExomuxFireField: keeps obstacle windows as cold voids the flames avoid", () => {
  ...
  const litInside = (field: ExomuxFireField): number => {
    const cells = field.rasterizeCells(BOUNDS, THEME);
    let count = 0;
  ...
  const avoiding = new ExomuxFireField({ seed: 31 });
  ...
  const covering = new ExomuxFireField({ seed: 31 });
  run(covering, 200, []);
  assert(litInside(covering) > 0, "a reclaimed window should catch fire");
});

Deno.test("ExomuxFireField: the pointer fans a hot bloom into the flames", () => {
  ...
  const hotIn = (field: ExomuxFireField): number => {
  ...
  const cold = new ExomuxFireField({ seed: 5 });
  run(cold, 120);
  const baseline = hotIn(cold);

  const fanned = new ExomuxFireField({ seed: 5 });
  run(fanned, 120);
  let now = 120 * 60;
...
Deno.test("ExomuxFireField: cell colors are valid RGB and follow the theme", () => {
  const field = new ExomuxFireField({ seed: 44 });
  run(field, 200);
...
Deno.test("ExomuxFireField: 100 advance+rasterize frames at 200x60 stay under budget", () => {
  const field = new ExomuxFireField({ seed: 5 });
