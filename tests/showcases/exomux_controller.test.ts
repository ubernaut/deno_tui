...
import { createExomuxController } from "../../examples/showcases/exomux/controller.ts";
import {
  EXOMUX_T2_SWATCHES,
  EXOMUX_THEMES,
...
import {
  launchInitialExomuxTerminalIfEmpty,
  parseExomuxShowcaseArgs,
} from "../../examples/showcases/exomux/main.ts";
import {
  exomuxTerminalForegroundRgb,
  exomuxTerminalRgb,
} from "../../examples/showcases/exomux/terminal_palette.ts";
...
Deno.test("Exomux restores theme and exact dynamic window placement while daemon sessions persist", async () => {
  ...
  try {
    ...
    assertEquals(second.theme.peek(), exomuxTheme("t2"));
