...
import { ExomuxIvyField } from "../../examples/showcases/exomux/ivy_background.ts";
...
function grid(field: ExomuxIvyField, theme = THEME): string[] {
  return field.rasterizeCells(BOUNDS, theme).map((row) => row.map((cell) => cell?.char ?? " ").join(""));
}

Deno.test("ExomuxIvyField: same seed and advance sequence are deterministic", () => {
  const left = new ExomuxIvyField({ seed: 21 });
  const right = new ExomuxIvyField({ seed: 21 });
  run(left, 900);
  run(right, 900);
  assertEquals(grid(left), grid(right));

  const other = new ExomuxIvyField({ seed: 22 });
  run(other, 900);
  assert(grid(other).join("\n") !== grid(left).join("\n"), "different seeds should diverge");
});

Deno.test("ExomuxIvyField: matches bounds dimensions and survives resizes", () => {
  const field = new ExomuxIvyField({ seed: 3 });
  run(field, 300);
...
Deno.test("ExomuxIvyField: strands creep outward along curving arcs", () => {
  const field = new ExomuxIvyField({ seed: 9 });
  run(field, 60);
...
Deno.test("ExomuxIvyField: sections mature through stalk, leaves, flowers, then fruit", () => {
  const field = new ExomuxIvyField({ seed: 15 });
...
Deno.test("ExomuxIvyField: ripe fruit is pickable and bursts into confetti", () => {
  const field = new ExomuxIvyField({ seed: 15 });
...
Deno.test("ExomuxIvyField: keeps clear of obstacles and creeps over them once released", () => {
  ...
  const inside = (field: ExomuxIvyField): number => {
  ...
  const avoiding = new ExomuxIvyField({ seed: 31 });
  ...
  const covering = new ExomuxIvyField({ seed: 31 });
  run(covering, 1_500, []);
  assert(inside(covering) > 0, "a reclaimed window should get ivy drawn across it");
});

Deno.test("ExomuxIvyField: cell colors are valid RGB and follow the theme", () => {
  const field = new ExomuxIvyField({ seed: 44 });
  run(field, 2_400);
...
Deno.test("ExomuxIvyField: 100 advance+rasterize frames at 200x60 stay under budget", () => {
  const field = new ExomuxIvyField({ seed: 5 });
