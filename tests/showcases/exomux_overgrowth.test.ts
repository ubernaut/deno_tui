...
} from "../../examples/showcases/exomux/overgrowth.ts";
import { EXOMUX_BACKGROUND_IDS } from "../../examples/showcases/exomux/model.ts";

const RECT = { column: 10, row: 4, width: 20, height: 10 };

Deno.test("exomux overgrowth applies only to the organic backgrounds", () => {
  assertEquals([...EXOMUX_OVERGROWTH_BACKGROUND_IDS].sort(), ["circuit", "fire", "ivy", "jungle", "matrix"]);
  for (const id of EXOMUX_BACKGROUND_IDS) {
    assertEquals(exomuxBackgroundOvergrows(id), EXOMUX_OVERGROWTH_BACKGROUND_IDS.includes(id));
  }
});
