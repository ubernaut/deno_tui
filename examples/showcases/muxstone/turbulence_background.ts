...
async function createTurbulenceGpu(width: number, height: number): Promise<TurbulenceGpuContext | null> {
  try {
    ...
    const macroBuffer = device.createBuffer({ size: macroSize, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC });
    const stagingBuffer = device.createBuffer({ size: macroSize, usage: GPUBufferUsage.MAP_READ | GPUBufferUsage.COPY_DST });
    ...
    return {
      ...
      fBufferA, fBufferB, obsBuffer, macroBuffer, stagingBuffer,
      bindGroupA: makeBindGroup(fBufferA, fBufferB),
      bindGroupB: makeBindGroup(fBufferB, fBufferA),
      width, height, ping: false, frame: 0,
      readbackPending: false, readbackData: null,
    };
  } catch {
    return null;
  }
...
interface TurbulencePointer extends MuxstoneBackgroundPoint {
  readonly updatedAt: number;
}

/** Construction options. */
export interface MuxstoneTurbulenceFieldOptions {
 ...
 */
export class MuxstoneTurbulenceField implements MuxstoneAnimatedBackground {
  #bounds?: Rectangle;
  ...
  constructor(_options: MuxstoneTurbulenceFieldOptions = {}) {}
  ...
  rasterizeCells(
    ...
    if (!normalized) { this.#cells = []; return this.#cells; }
    ...
    const cwColor: MuxstoneRgb = mixMuxstoneRgb(theme.accent, theme.text, 0.3);
    const ccwColor: MuxstoneRgb = mixMuxstoneRgb(theme.muted, theme.text, 0.3);
    ...
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < width; x += 1) {
        ...
        const foreground = mixMuxstoneRgb(theme.background, hue, intensity);
