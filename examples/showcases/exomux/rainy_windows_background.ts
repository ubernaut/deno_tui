 ...
 */
export class ExomuxRainyWindowsField implements ExomuxOverlayBackground {
  #randomState: number;
  readonly #density: number;
  #bounds?: Rectangle;
  ...
  rasterizeCells(
    ...
    for (const drop of this.#drops) {
      ...
      for (let offset = 0; offset <= drop.tail; offset += 1) {
        ...
        this.#cells[row]![column] = { char: glyph, foreground: mixExomuxRgb(rainBody, theme.background, fade) };
      }
    }
  ...
  rasterizeOverlayCells(
    ...
    const { width, height } = normalized;
    const obstacle = this.#activeObstacle;
    const overlay: ExomuxOverlayCell[] = [];
    ...
    for (const splash of this.#splashes) {
      ...
      for (const arc of SPLASH_ARC_TEMPLATES) {
        ...
        const foreground = mixExomuxRgb(theme.background, rainBright, alpha);
    ...
    if (obstacle && this.#water.length > 0) {
      ...
      if (puddleRow >= 0 && puddleRow < height) {
        const puddleColor = mixExomuxRgb(rainBright, theme.text, 0.4);
        for (let i = 0; i < this.#water.length; i += 1) {
          ...
          const foreground = mixExomuxRgb(theme.background, puddleColor, alpha);
    ...
    const drizzleHead: ExomuxRgb = mixExomuxRgb(rainBright, theme.text, 0.3);
    const drizzleTail: ExomuxRgb = mixExomuxRgb(rainBody, theme.background, 0.15);
    for (const drizzle of this.#drizzles) {
      ...
      for (let t = 0; t <= drizzle.tail; t += 1) {
        ...
        const char = t === 0 ? "#" : t === 1 ? "=" : "~";
        const fg = mixExomuxRgb(drizzleHead, drizzleTail, fade);
