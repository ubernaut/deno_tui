...
interface JunglePointer extends ExomuxBackgroundPoint {
 ...
 * previous frame.
 */
export class ExomuxJungleField implements ExomuxAnimatedBackground {
  #randomState: number;
  ...
  #paintLayer(layer: JungleLayer, depth: number, bounds: Rectangle, theme: ExomuxThemeSpec): void {
  ...
  #leafCell(
    ...
    if (depth === 0) {
      return { char, foreground: mixExomuxRgb(theme.background, theme.muted, 0.3 + 0.3 * shade + 0.15 * rustle) };
    }
    if (depth === 1) {
      const blend = mixExomuxRgb(theme.muted, theme.success, 0.35 + 0.65 * shade);
      return { char, foreground: mixExomuxRgb(theme.background, blend, 0.6 + 0.15 * shade + 0.2 * rustle) };
    }
    const base = mixExomuxRgb(theme.success, theme.accent, highlight * 0.85);
    const visible = mixExomuxRgb(base, theme.text, FRONT_TEXT_BLEND);
