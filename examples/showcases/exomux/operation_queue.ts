...
const MAX_BUFFERED_INPUT_BYTES = EXOMUX_PROTOCOL_LIMITS.inputBytes * INPUT_PIPELINE_DEPTH;
const inputEncoder = new TextEncoder();

type ExomuxPreparedInput =
  | Readonly<{ kind: "empty" }>
  | Readonly<{ kind: "rejected" }>
  | Readonly<{ kind: "accepted"; bytes: Uint8Array }>;

interface ExomuxOperationQueueOptions {
  readonly write: (sessionId: string, data: Uint8Array) => Promise<boolean>;
  readonly reportError: (error: unknown) => void;
}

interface ExomuxPendingInput {
  readonly sessionId: string;
  readonly bytes: Uint8Array;
}

interface ExomuxInputQueueEntry {
  readonly kind: "input";
  readonly pending: ExomuxPendingInput[];
  readonly waiters: Array<() => void>;
  inFlight: number;
}

interface ExomuxBarrierQueueEntry {
...
type ExomuxQueueEntry = ExomuxInputQueueEntry | ExomuxBarrierQueueEntry;
 ...
 */
export class ExomuxOperationQueue {
  readonly #write: (sessionId: string, data: Uint8Array) => Promise<boolean>;
  readonly #reportError: (error: unknown) => void;
  readonly #entries: ExomuxQueueEntry[] = [];
  ...
  constructor(options: ExomuxOperationQueueOptions) {
  ...
  #prepareInput(data: string | Uint8Array): ExomuxPreparedInput {
...
function appendTargetedProtocolChunks(
  ...
  if (
    ...
    previous.bytes.byteLength < EXOMUX_PROTOCOL_LIMITS.inputBytes
  ) {
    const take = Math.min(bytes.byteLength, EXOMUX_PROTOCOL_LIMITS.inputBytes - previous.bytes.byteLength);
  ...
  while (offset < bytes.byteLength) {
    const end = Math.min(bytes.byteLength, offset + EXOMUX_PROTOCOL_LIMITS.inputBytes);
...
function appendProtocolChunks(pending: Uint8Array[], bytes: Uint8Array): void {
  let offset = 0;
  const previous = pending.at(-1);
  if (previous && previous.byteLength < EXOMUX_PROTOCOL_LIMITS.inputBytes) {
    const take = Math.min(bytes.byteLength, EXOMUX_PROTOCOL_LIMITS.inputBytes - previous.byteLength);
