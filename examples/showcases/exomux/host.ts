...
export class ExomuxHostController {
  ...
  constructor(options: ExomuxHostControllerOptions) {
    ...
    this.#backendFactory = backend ? () => backend : options.backendFactory ?? createDefaultExomuxTerminalBackend;
  }
...
export async function createDefaultExomuxTerminalBackend(
  ...
  try {
    return await createPtyBackend({ pollingIntervalMs: EXOMUX_PTY_POLLING_INTERVAL_MS });
  } catch {
