 ...
 */
export function exomuxOvergrowthThreshold(column: number, row: number, rect: Rectangle): number {
...
export function exomuxOvergrowthCovers(
  ...
  return exomuxOvergrowthThreshold(column, row, rect) < ratio;
}
...
export function exomuxOvergrowthVisible(
  ...
  if (!exomuxOvergrowthCovers(column, row, rect, ratio)) return false;
