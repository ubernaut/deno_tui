...
interface VaporwavePointer extends ExomuxBackgroundPoint {
 ...
 * identical grids.
 */
export class ExomuxVaporwaveField implements ExomuxAnimatedBackground {
  #randomState: number;
  readonly #pointerLifetimeMs: number;
  #bounds?: Rectangle;
  ...
  constructor(options: ExomuxVaporwaveFieldOptions = {}) {
  ...
  inspect(): ExomuxVaporwaveInspection {
    return {
  ...
  #paintSky(
    ...
    const horizonBlend = mixExomuxRgb(theme.danger, theme.surface, 0.5);
    const skyColor = (row: number): ExomuxRgb => {
      const t = horizonRow <= 1 ? 1 : row / (horizonRow - 1);
      return mixExomuxRgb(theme.background, horizonBlend, 0.45 * Math.pow(t, 1.6));
    };
    ...
    for (let band = 0; band < this.#hazeRows.length; band += 1) {
      ...
      const foreground = mixExomuxRgb(skyColor(row), theme.muted, 0.45);
      for (let column = 0; column < width; column += 1) {
    ...
    for (const star of this.#stars) {
      ...
      this.#cells[star.row]![star.column] = {
        ...
        foreground: mixExomuxRgb(skyColor(star.row), theme.muted, 0.35 + 0.45 * star.brightness),
      };
    }
  }
  ...
  #paintSun(
    ...
    for (let row = topRow; row < horizonRow; row += 1) {
      ...
      const foreground = mixExomuxRgb(theme.warning, theme.danger, gradient);
  ...
  #paintFloor(
    ...
    if (horizonRow < height) {
      const foreground = mixExomuxRgb(theme.accent, theme.danger, 0.4);
      for (let column = 0; column < width; column += 1) {
    ...
    for (let index = -maxIndex; index <= maxIndex; index += 1) {
      for (let row = horizonRow + 1; row < height; row += 1) {
        ...
        this.#cells[row]![column] = {
          ...
          foreground: mixExomuxRgb(theme.accent, theme.background, fade),
        };
      }
    }
    ...
    for (const depth of this.#lineDepths) {
      ...
      const lineColor = mixExomuxRgb(theme.accent, theme.background, fade);
      const crossColor = mixExomuxRgb(lineColor, theme.text, 0.35);
