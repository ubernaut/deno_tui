...
export interface ExomuxSkullFieldOptions {
...
export interface ExomuxSkullEyeInspection {
...
export interface ExomuxSkullInspection {
  readonly bounds?: Rectangle;
  readonly pointer?: ExomuxBackgroundPoint & { readonly updatedAt: number };
  ...
  readonly eyes: readonly ExomuxSkullEyeInspection[];
}

interface SkullPointer extends ExomuxBackgroundPoint {
 ...
 */
export class ExomuxSkullField implements ExomuxAnimatedBackground {
  readonly #seed: number;
  readonly #pointerIdleMs: number;
  #randomState: number;
  #bounds?: Rectangle;
  ...
  constructor(options: ExomuxSkullFieldOptions = {}) {
  ...
  rasterizeCells(
    ...
    const boneCore = mixExomuxRgb(theme.text, theme.accent, 0.1);
    const boneMid = mixExomuxRgb(theme.muted, theme.border, 0.35);
    const boneEdge = mixExomuxRgb(theme.border, theme.background, 0.45);
    const outline: ExomuxBackgroundCell = {
      char: "█",
      foreground: mixExomuxRgb(theme.background, theme.border, 0.22),
    };
    const socket: ExomuxBackgroundCell = {
      char: "█",
      foreground: mixExomuxRgb(theme.background, theme.muted, 0.05),
    };
    const socketRim: ExomuxBackgroundCell = {
      char: "█",
      foreground: mixExomuxRgb(theme.background, theme.border, 0.11),
    };
    const iris: ExomuxBackgroundCell = {
      char: "█",
      foreground: mixExomuxRgb(theme.warning, theme.text, 0.2),
      bold: true,
    };
    const nasal: ExomuxBackgroundCell = {
      char: "▓",
      foreground: mixExomuxRgb(theme.background, theme.muted, 0.08),
    };
    const eyelid: ExomuxBackgroundCell = {
      char: "▓",
      foreground: mixExomuxRgb(boneEdge, theme.background, 0.25),
    };
    const toothLit = mixExomuxRgb(theme.text, theme.muted, 0.28);
    const toothGap = mixExomuxRgb(theme.background, theme.muted, 0.22);
        ...
        switch (layout.mask[index]) {
          case REGION_FILL: {
            ...
            const base = depth < 0.5
              ? mixExomuxRgb(boneEdge, boneMid, depth / 0.5)
              : mixExomuxRgb(boneMid, boneCore, (depth - 0.5) / 0.5);
            const edgeShade = Math.max(0, 0.5 - depth) * (0.32 + 0.14 * breathWave);
            const color = edgeShade > 0 ? mixExomuxRgb(base, theme.background, edgeShade) : base;
            const detail = layout.overlayChar[index];
            row[x] = detail !== undefined
              ? { char: detail, foreground: mixExomuxRgb(color, theme.background, 0.5) }
          ...
          case REGION_NASAL:
            ...
            break;
          case REGION_BROW:
            row[x] = { char: "▓", foreground: mixExomuxRgb(theme.background, theme.muted, browShade) };
            break;
          case REGION_TEETH: {
            const d = layout.level[index]! / 250;
            row[x] = { char: d > 0.5 ? "█" : d > 0.24 ? "▓" : "▒", foreground: mixExomuxRgb(toothGap, toothLit, d) };
            break;
          }
          default:
  ...
  #machineryCell(
    ...
    breath: number,
    seconds: number,
  ): ExomuxBackgroundCell | undefined {
    const glyph = layout.overlayChar[index];
    if (glyph !== undefined) {
      const depth = layout.overlayDepth[index]! / 255;
      const structure = mixExomuxRgb(theme.border, theme.muted, depth);
      const tubeRef = layout.overlayTube[index]!;
      if (tubeRef > 0) {
        const pulse = tubePulse(layout.tubes[tubeRef - 1]!, layout.overlayPos[index]!, seconds);
        let color = mixExomuxRgb(theme.background, structure, Math.min(1, (0.26 + 0.4 * depth) * breath));
        if (pulse > 0) color = mixExomuxRgb(color, theme.accent, 0.65 * pulse);
        return pulse > 0.75 ? { char: glyph, foreground: color, bold: true } : { char: glyph, foreground: color };
      }
      let color = mixExomuxRgb(theme.background, structure, Math.min(1, (0.24 + 0.42 * depth) * breath));
      if (depth > 0.85) color = mixExomuxRgb(color, theme.accent, 0.3);
    ...
    return {
      char: depth < 0.55 ? "░" : "▒",
      foreground: mixExomuxRgb(theme.background, structure, Math.min(1, (0.14 + 0.38 * depth) * breath)),
    };
  }

  #paintPupils(layout: SkullLayout, theme: ExomuxThemeSpec): void {
    const pupil: ExomuxBackgroundCell = {
      char: "█",
      foreground: mixExomuxRgb(theme.background, theme.border, 0.06),
    };
    const glint: ExomuxBackgroundCell = {
      char: "█",
      foreground: mixExomuxRgb(theme.text, theme.warning, 0.08),
      bold: true,
    };
  ...
  #paintIrisCell(layout: SkullLayout, x: number, y: number, cell: ExomuxBackgroundCell): boolean {
  ...
  #prepareCellBuffer(bounds: Rectangle): void {
    if (this.#cells.length !== bounds.height || (this.#cells[0]?.length ?? -1) !== bounds.width) {
      this.#cells = Array.from(
        ...
        () => new Array<ExomuxBackgroundCell | undefined>(bounds.width),
      );
    }
